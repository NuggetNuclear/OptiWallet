...
interface MerchantSearchProps {
  ...
  /** Si true, ordena por popularidad (prior desc); si false, alfabético. Default true. */
  sortByPopularity?: boolean;
}

export function MerchantSearch({ onSelect, sortByPopularity = true }: MerchantSearchProps) {
  ...
  // así que el toggle no debe re-pegarle al server. Con popularidad: prior desc y
  // alfabético como desempate (comercios sin correr el script empatan en 0.5 →
  // caen a A-Z, degradación limpia). Sin toggle: alfabético puro.
  const sortedMerchants = useMemo(() => {
    const copy = [...merchants];
    copy.sort((a, b) =>
      sortByPopularity
        ? b.popularity_prior - a.popularity_prior || a.name.localeCompare(b.name)
        : a.name.localeCompare(b.name),
    );
    return copy;
  }, [merchants, sortByPopularity]);

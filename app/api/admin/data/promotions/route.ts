...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const rows = await sql`
      ...
      ORDER BY COALESCE(p.discount, 0) DESC, p.id
    `;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    `;
    const descLabel = discount != null ? `${discount}%` : `$${discount_per_unit}/L`;
    await logAdminAction(session, "create", "promotion", id, `Promoción ${descLabel} en banco ${bank_id} / comercio ${merchant_id}`, clientIp(req));

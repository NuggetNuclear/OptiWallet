...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (next.available === true) {
      const cards = await sql`SELECT id FROM cards WHERE bank_id = ${id} LIMIT 1`;
      if (!cards.length) {
        return NextResponse.json(
          { error: "No puedes activar este banco porque no tiene tarjetas asociadas. Agrega al menos una tarjeta primero." },

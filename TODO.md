...
- [ ] **Plausible**: agregar el sitio en plausible.io y setear `NEXT_PUBLIC_PLAUSIBLE_SRC` (el `src` del snippet v2) en Vercel.
...
- [x] **Bandeja de reportes de usuarios**: el 👎 en las recomendaciones ahora persiste en `promo_reports` (captura en dos fases, motivo opcional) en vez de solo disparar un evento de analítica. La captura pública vive en este repo (`POST`/`PATCH /api/promo-reports`); el triage por promo (resolver/descartar/desactivar), con priorización heurística por defecto y opcional por IA, vive en el panel admin (repo separado `Optiwallet-admin`).
...
- [x] Fetch de Banco de Chile con progreso en vivo (SSE) en una consola tipo terminal (`TerminalConsole`) en vez de un spinner opaco. Vive en el panel admin (repo separado `Optiwallet-admin`).
